...
write_json(list(version=unbox(paste(biocVersion))), path="bioc-info.json", pretty=TRUE)
...
updatePackageSet <- function(mirrorType) {
  ...
  cl <- makeCluster(10)
  clusterExport(cl, varlist=c("escapeName", "nixPrefetch", "prevPkgs", "mirrorUrl", "mirrorType", "knownPackageNames"), envir=environment())
  ...
  write_json(c(pkgs, brokenPkgs), path=packagesFile, pretty=TRUE)
  write("done", stderr())
}

res <- lapply(names(mirrorUrls), updatePackageSet)
